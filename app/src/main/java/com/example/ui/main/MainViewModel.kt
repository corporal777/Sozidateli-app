...
@Inject constructor(
    ...
    private val _eventAgreement = mutableStateOf<EventModel?>(null)
    val eventAgreement: State<EventModel?> = _eventAgreement
    ...
    init {
        ...
        uiStateData.eventAgreement
            .onEach { _eventAgreement.value = it }
            .launchIn(viewModelScope)
    }
...
//            .launchIn(viewModelScope)
    ...
    fun sendEventAgreement(event: EventModel?) {
        viewModelScope.launch {
            _eventAgreement.value = null
            uiStateData.changedEvent.emit(event)
        }
    }
}