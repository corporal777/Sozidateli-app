package com.example.ui.gallery.cropImage.cropHelper;
...
import com.example.ui.gallery.cropImage.cropHelper.animation.SimpleValueAnimator;
import com.example.ui.gallery.cropImage.cropHelper.animation.SimpleValueAnimatorListener;
import com.example.ui.gallery.cropImage.cropHelper.animation.ValueAnimatorV14;
import com.example.ui.gallery.cropImage.cropHelper.animation.ValueAnimatorV8;
import com.example.ui.gallery.cropImage.cropHelper.callback.Callback;
import com.example.ui.gallery.cropImage.cropHelper.callback.CropCallback;
import com.example.ui.gallery.cropImage.cropHelper.callback.LoadCallback;
import com.example.ui.gallery.cropImage.cropHelper.callback.SaveCallback;
import com.example.ui.gallery.cropImage.cropHelper.util.Logger;
...
@SuppressWarnings("unused") public class CropImageView extends AppCompatImageView {
    ...
    private void handleStyleable(Context context, AttributeSet attrs, int defStyle, float mDensity) {
        ...
        try {
            ...
            for (CropMode mode : CropMode.values()) {
                if (ta.getInt(R.styleable.CropImageView_scv_crop_mode, 3) == mode.getId()) {
                    mCropMode = mode;
                    break;
                }
            }
            mBackgroundColor =
                    ta.getColor(R.styleable.CropImageView_scv_background_color, TRANSPARENT);
            mOverlayColor =
                    ta.getColor(R.styleable.CropImageView_scv_overlay_color, TRANSLUCENT_BLACK);
            ...
            for (ShowMode mode : ShowMode.values()) {
                if (ta.getInt(R.styleable.CropImageView_scv_guide_show_mode, 1) == mode.getId()) {
                    mGuideShowMode = mode;
                    break;
                }
            }
            ...
            for (ShowMode mode : ShowMode.values()) {
                if (ta.getInt(R.styleable.CropImageView_scv_handle_show_mode, 1) == mode.getId()) {
            ...
            mHandleSize = ta.getDimensionPixelSize(R.styleable.CropImageView_scv_handle_size,
                    (int) (HANDLE_SIZE_IN_DP * mDensity));
            mTouchPadding = ta.getDimensionPixelSize(R.styleable.CropImageView_scv_touch_padding, 0);
            mMinFrameSize = ta.getDimensionPixelSize(R.styleable.CropImageView_scv_min_frame_size,
                    (int) (MIN_FRAME_SIZE_IN_DP * mDensity));
            mFrameStrokeWeight =
                    ta.getDimensionPixelSize(R.styleable.CropImageView_scv_frame_stroke_weight,
                            (int) (FRAME_STROKE_WEIGHT_IN_DP * mDensity));
            mGuideStrokeWeight =
                    ta.getDimensionPixelSize(R.styleable.CropImageView_scv_guide_stroke_weight,
                            (int) (GUIDE_STROKE_WEIGHT_IN_DP * mDensity));
            mIsCropEnabled = ta.getBoolean(R.styleable.CropImageView_scv_crop_enabled, true);
            mInitialFrameScale = constrain(
                    ta.getFloat(R.styleable.CropImageView_scv_initial_frame_scale,
                            DEFAULT_INITIAL_FRAME_SCALE), 0.01f, 1.0f, DEFAULT_INITIAL_FRAME_SCALE);
            mIsAnimationEnabled =
                    ta.getBoolean(R.styleable.CropImageView_scv_animation_enabled, true);
            mAnimationDurationMillis = ta.getInt(R.styleable.CropImageView_scv_animation_duration,
                    DEFAULT_ANIMATION_DURATION_MILLIS);
            mIsHandleShadowEnabled =
                    ta.getBoolean(R.styleable.CropImageView_scv_handle_shadow_enabled, true);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            ta.recycle();
    ...
    private Bitmap getImage(Uri sourceUri) {
        ...
        if (sourceUri == null) throw new IllegalStateException("Source Uri must not be null.");
        ...
        sampledBitmap = Utils.decodeSampledBitmapFromUri(getContext(), mSourceUri, requestSize);
        mInputImageWidth = Utils.sInputImageWidth;
        mInputImageHeight = Utils.sInputImageHeight;

//        try {
//            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
//                sampledBitmap = ImageDecoder.decodeBitmap(ImageDecoder.createSource(getContext().getContentResolver(), sourceUri));
//            } else {
//                sampledBitmap = MediaStore.Images.Media.getBitmap(getContext().getContentResolver(), sourceUri);
//            }
//        } catch (IOException e) {
//            e.printStackTrace();
//        }
