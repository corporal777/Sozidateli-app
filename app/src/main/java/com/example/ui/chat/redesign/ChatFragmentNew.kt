package com.example.ui.chat.redesign
...
import com.example.ui.chat.ChatPresenter
...
class ChatFragmentNew : BaseFragmentNew<FragmentChatBinding>(), ChatContractNew.View {

    private var animCounter = 0

    @Inject
    lateinit var presenterProvider: Provider<ChatPresenterNew>

    @InjectPresenter(type = PresenterType.WEAK)
    lateinit var presenter: ChatPresenterNew

    @ProvidePresenterTag(presenterClass = ChatPresenterNew::class, type = PresenterType.WEAK)
    fun provideRepositoryPresenterTag(): String? {
        return chatId
    }

    @ProvidePresenter(type = PresenterType.WEAK)
    fun providePresenter(): ChatPresenterNew = presenterProvider.get().apply {
        val presenter = this
        requireArguments().let { ChatFragmentNewArgs.fromBundle(it) }.apply {
    ...
    val chatId: String?
        get() = arguments?.let { ChatFragmentNewArgs.fromBundle(it).chatId }
