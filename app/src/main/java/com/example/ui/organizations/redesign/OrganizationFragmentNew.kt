...
import com.example.databinding.FragmentOrganizationNewBinding
...
import com.example.ui.organizations.OrganizationFragmentDirections
...
class OrganizationFragmentNew : BaseFragmentNew<FragmentOrganizationNewBinding>(),
    OrganizationContractNew.View,
    SimpleTitleToolbar {

    @InjectPresenter
    lateinit var presenter: OrganizationPresenterNew

    @Inject
    lateinit var presenterProvider: Provider<OrganizationPresenterNew>

    @ProvidePresenter
    fun providePresenter(): OrganizationPresenterNew = presenterProvider.get().apply {
        organizationId = OrganizationFragmentNewArgs.fromBundle(requireArguments()).organizationId
    }
    ...
    override fun updateUserSubscription(userId: Int, isSubscribed: Boolean) {
        ...
        val item = membersDataSection.findItemBy<UserItemNew> { userItem -> userItem.id == idLong } ?: return
        item.notifyChanged(isSubscribed)
    }
    ...
    override fun layout(): Int = R.layout.fragment_organization_new
