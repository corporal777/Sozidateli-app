...
class InviteListFragment : BaseFragment<FragmentInviteListBinding>(), InviteListContract.View {
    ...
    override fun setInvitesData(chats: List<UserChat?>) {
        if (chats.isEmpty())
            invitesSection.updateItem(NoScheduleEventItem(getString(R.string.empty_list_placeholder_message), padding = 70.dp))
        else invitesSection.updateItems(
            ListSectionNameItem(-300L, getString(R.string.chat_list_chat_requests)),
