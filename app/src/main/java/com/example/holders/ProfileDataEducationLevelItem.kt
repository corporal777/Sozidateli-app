...
) : BindableItem<ItemProfileDataEducationLevelBinding>() {
        ...
        viewBinding.apply {
            ...
            if ((academicDegrees.size == 1 && academicDegrees[0].degree == 0) || academicDegrees.isEmpty()) {
                ...
                if (sp.isEmpty()) {
                    ...
                    tvSpecializationLevel.text =
                        academicDegrees.joinToString(separator = "\n") { degree ->
                            val degre =
                                availableSpecialities.firstOrNull { item -> item.id == degree.speciality }?.name
                            degre ?: ""
                        }
