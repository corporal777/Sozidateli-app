...
import com.generator.qrcodegenerator.style.EmptyDrawable
...
//
    ...
    private fun applyNaturalLogo(logoBgSize: Int, size: Float, pixelSize: Float) {
    ...
    private fun createLogo(logoSize: Float): Bitmap? {
        if (options.logo.bitmap == null && options.logo.drawable == null) return null

        val bitmapLogo = options.logo.scale.scale(options.logo, logoSize, logoSize)
        bitmapLogo.applyCanvas {
            val clip = Path().apply {
        ...
        return when (options.logo.shape) {
            is QrVectorLogoShape.RoundCorners -> {
                getRoundedCornerBitmap(bitmapLogo, options.logo.shape.radius.toInt())
            }
            is QrVectorLogoShape.Circle -> getRoundedCornerBitmap(bitmapLogo, 100)
            else -> bitmapLogo
        }
    }
